...
class RouteOptimizerBrowaeys:
    ...
    def update_particle(self,particle_num,new_loc):             #Hoe werkt je sequence van particles? begin je in 0,0 tot 0,7, dan rij 1 etc?
        if new_loc in self.particles.tolist():  
            print("Already particle in spot: "+str(new_loc))
        else:
            self.particles[particle_num,:]=new_loc
            
    def number_particles_in_block(self,k,l):                    #wat definieër je hier met block?
        """
    ...
    def calc_distance_punish(self,k,l):
        ...
        for x in range(minx-2,maxx+2):                                          #Deze for loop werkt niet met minx als float64
    ...
    def calc_costs(self,p,mode,lambda_adjusted=1,dim_in=2,center=[0,0]):
        ...
        if mode=="punish":
            ...
            for k in range(self.num_template):
                for l in range(self.num_particle):
                    self.cost_matrix[k,l]=self.calc_distance(p,k,l)
                    if (not self.cost_matrix[k,l]==0) and self.cost_matrix[k,l]<6:            #Waarom deze toevoeging? en waar komt 6 vandaan?
                        ...
                        for x in range(int(min(self.particles[k][0],self.template[l][0])),int(max(self.particles[k][0],self.template[l][0])+1)):        #-
                            for y in range(int(min(self.particles[k][1],self.template[l][1])),int(max(self.particles[k][1],self.template[l][1])+1)):    #iterate over inner grid
                                ...
                                if self.particle_matrix[x,y]==1 and ([x,y]!=self.particles[k]).any():               #excluding the two initial template and particle points
                                    in_sum+=1
                        self.cost_matrix[l,k]*=(in_sum/array_sum) #normalized over amount of points in inner grid (-1 because of ignoring the particle itself)
...
# =============================================================================
    ...
    def available_spot_route(self,start,target,mindim,maxdim,recdepth,maxdepth):            #waarom gebruik je dit? Hier verander je toch alleen de int naar string
        """
    ...
    def route_shortener(self,route):        #Komt dit vaak voor? If so; is het niet simpeler om je route finding algoritme te optimaliseren?
        """
    ...
    def create_assignment_sinkhornn(self, reg, mapping):
        ...
        if mapping=="BvN_max_alpha":
            ...
            #tf.print(gamma_tensor, summarize=-1)
            #x = tf.fill([1, 5, 5], 0.2, name='doubly_stochastic')
...
# =============================================================================
    ...
    def get_mean_std(self,data):
        ...
        for i in range(len(data[:,0,0])):
            means[i] = self.exp_weighted_mean(data[i,:,0],data[i,:,1],500)
